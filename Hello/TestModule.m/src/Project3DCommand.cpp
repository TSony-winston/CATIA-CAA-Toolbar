...
HRESULT GenerateView(CATIView* iView, CATIDftViewMakeUp* iMakeUp, CATILinkableObject* iPart,
...
HRESULT CreateProjection(CATIDrwFactory* iFactory, CATISheet_var iSheet, CATILinkableObject* iPart,
                         CATDrwViewType iType, const char* iName, double iX, double iY,
                         const CATMathVector& iHorizontal, const CATMathVector& iVertical) {
    ...
    if (SUCCEEDED(hr) && pView != NULL) {
        ...
        if (generSpec != NULL_var) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                if (SUCCEEDED(pView->QueryInterface(IID_CATIAlias, (void**)&pAlias)) &&
                    pAlias != NULL) {
                ...
                if (FAILED(hr)) {
                    ShowProject3DMessage("CATIA could not define or update the generative view "
                ...
                if (itemCount == 0) {
                    hr = E_FAIL;
                    ShowProject3DMessage("The drawing view contains no generated geometry. The "
                                         "projection is incomplete.");
                }

                CATIModelEvents* pEvents = NULL;
                if (SUCCEEDED(iSheet->QueryInterface(IID_CATIModelEvents, (void**)&pEvents)) &&
                    pEvents != NULL) {
...
HRESULT ProjectPartInDrawing(CATDocument* iDrawingDocument, CATDocument* iPartDocument) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (sheet == NULL_var)
            ...
            hr = CreateProjection(pDrawingFactory, sheet, pPartLink, FrontView, "Front View", 110.0,
                                  90.0, CATMathVector(1.0, 0.0, 0.0), CATMathVector(0.0, 0.0, 1.0));
            if (SUCCEEDED(hr))
                hr = CreateProjection(pDrawingFactory, sheet, pPartLink, TopView, "Top View", 110.0,
                                      150.0, CATMathVector(1.0, 0.0, 0.0),
                                      CATMathVector(0.0, 1.0, 0.0));
            if (SUCCEEDED(hr))
                hr = CreateProjection(pDrawingFactory, sheet, pPartLink, LeftView, "Left View",
                                      50.0, 90.0, CATMathVector(0.0, -1.0, 0.0),
                                      CATMathVector(0.0, 0.0, 1.0));
            CATIRedrawEvent* pRedraw = NULL;
            if (SUCCEEDED(sheet->QueryInterface(IID_CATIRedrawEvent, (void**)&pRedraw)) &&
                pRedraw != NULL) {
                pRedraw->Redraw();
                pRedraw->Release();
            }
...
CATStatusChangeRC Project3DCommand::Activate(CATCommand* iFromClient, CATNotification* iEvent) {
    ...
    if (pPart == NULL) {
        ...
        if (SUCCEEDED(hr)) {
            ShowProject3DMessage(
                "Front, top, and left views were generated from the last created part.");
        }
        else {
            ShowProject3DMessage(
                "Could not generate all three views. Activate a CATDrawing and ensure "
                "the last created part is still open and contains a solid. Partial views may "
                "remain.");
        }
    }
